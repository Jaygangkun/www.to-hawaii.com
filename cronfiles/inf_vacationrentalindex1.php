...
$sqlindexfeed = "SELECT `id`, `unitUrl` FROM `vacationrentalindexfeed` WHERE enabled=1 and `isnew`=0 and `unitUrl` in ( select `unitUrl` from vacationrentalindex where postalCode='0') limit 1000";
	   ...
	   $ch = curl_init();
		...
		foreach ($objResponse->photos as $photo) {
			 foreach ($photo->images as $selectImage) {
				if($selectImage->treatment=="1024x768")
				 {
		...
		foreach ($objResponse->photos as $photo) {
			 foreach ($photo->images as $selectImage) {
				if($selectImage->treatment=="200x150")
				 {
		...
		{
			$sqlQryForInsert="INSERT INTO `vacationrentalindex`(`uniturl`, `bathrooms`, `bedrooms`, `sleeps`, `href`, `thumbnailUrl`, `headline`, `description`, `state`, `city`, `country`, `lastModified`, `region`, `source`, `property_type`, `location_type`, `suitability`, `features`, `address`, `price`, `minimum_nights`, `rating`, `review_count`,`latitude`, `longitude`, `en_US_VR`, `en_US_VRBO`,`propertyName`,`actual_thumbnail`,`postalCode`) VALUES ('$uniturl',$bathrooms,$bedrooms,$sleeps,'$href','$thumbnailUrl','$headline','$description','$state','$city','$country','$lastModified','$region','$source','$property_type','$location_type','$suitability','$features','$address',$price,$minimum_nights,$rating,$review_count,'$latitude','$longitude','$en_US_VR','$en_US_VRBO','$propertyName','$actual_thumbnail','$postalCode');";
